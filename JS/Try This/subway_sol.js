...
class Subway {
  ...
  get isEnd() {
    return (this.rotateCnt === 2 && this.currIdx === this.startIdx - 1) || (this.end && LINE2[this.currIdx] === this.end);
  }
  ...
  iterator() {
    return {
      next: () => {
        ...
        const value = `${this.rotateCnt * LINE2.length + this.currIdx - this.startIdx + 2} - ${this.gotoNextStation()}`;
